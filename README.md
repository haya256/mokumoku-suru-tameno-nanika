...
同じWi-Fi（LAN）にいる人には、あなたのパソコンのIPアドレスを使ったURLを教えます。

```bash
# IPアドレスの調べ方（例: 192.168.1.23 のように表示されます）
...
トンネルのURLは長くてランダムな文字列なので、そのままだと参加者に伝えにくいことがあります。短縮URLサービスの TinyURL を使うと、`https://tinyurl.com/好きな名前` という形のURLにできます（アカウント登録なし・無料）。

1. ブラウザで https://tinyurl.com を開きます
2. 「Enter a long URL」の欄に、トンネルのURL（`https://ほにゃらら.trycloudflare.com`）を貼り付けます
...
- トンネルのURLは毎回変わるため、短縮URLももくもく会のたびに作り直します。古い短縮URLは、もうつながらないトンネルを指したまま残り続けます
