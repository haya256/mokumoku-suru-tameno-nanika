...
- 別の場所で開かれているもくもく会とつなぐと、お互いの様子とチャットが見える「ワールドマップ」になります（任意）
...
- 主催者が他のもくもく会とつないでいる場合、画面が3×3の「ワールドマップ」になります。真ん中が自分たちの部屋、周りがつながった相手の部屋です。マスをクリックするとその部屋が大きく表示され、真ん中をクリックすればいつもの表示に戻ります
  - このとき、**自分の発言は相手の部屋の参加者にも見えます**。逆に相手の発言や参加者も自分の画面に混ざって表示されます
...
| `world.peers` | つないだ他のもくもくルームの一覧。手で書くものではなく、Web画面の「ワールド設定」から接続すると自動的に書き込まれます。詳しくは後述の「ワールドマップ」を参照 |
...
- 入室・チャットなどでいつもの合言葉を聞かれたとき、**代わりにこの管理者合言葉を入力**すると、通常の書き込み権限に加えて「強制退出」「部屋画像の変更」「他のルームとの接続」権限も得られます（専用のログイン画面はありません）
...
- 参加者に配る合言葉とは別物にしてください。同じにすると全員が強制退出・部屋画像の変更・他ルームとの接続をできてしまいます

---

## ワールドマップ：他のもくもくルームとつなぐ（任意）

別の場所で開かれているもくもく会と、ゆるくつながることができます。つなぐと画面が3×3の「ワールドマップ」になり、真ん中が自分たちの部屋、周囲8マスのどこかに相手の部屋が現れます。マスをクリックするとその部屋がフルサイズで開きます。

つないだ相手の部屋については、次のものが自分の画面に出るようになります。
...
3. 相手ルームのURL（`https://xxxxx.trycloudflare.com` のような形）、画面に出す表示名（任意）、種別を入れて **「つなぐ」**

種別は基本「通常」のままで構いません。相手がこのリポジトリ系統（Flask）ではなく、[elm200版](https://github.com/elm200/mokumoku-suru-tameno-nanika)のようなFastAPI+Redisの派生版で動いている場合だけ「elm200 fork版」を選びます。この読み替えは向こうの現行のAPI形状に合わせた個別対応なので、向こうの実装が変わると通知なく壊れる可能性があります。またアクセス1回あたり相手のRedisコマンドを複数消費する作りのため、通常型より巡回間隔を空けています（既定60秒、`world.fork_poll_interval_sec` で調整可）。もくもく会は限られた時間だけ動かす運用が前提なのでこの間隔でも実用上問題ありませんが、長時間つなぎっぱなしにする場合は間隔を大きくすることを検討してください。

解除も同じパネルから「解除」ボタンで行えます。**接続・解除ともサーバーの再起動は不要**で、参加者の画面もページを開き直さずに追随します。
...
- 接続先は `config/settings.json` の `world.peers` に保存されるので、サーバーを再起動しても同じ相手・同じマスのままです
- 「elm200 fork版」でつないだ相手は、通常型（5秒間隔）と違って `world.fork_poll_interval_sec`（既定60秒）でしか状態を取りに行かないため、在室者やチャットの反映が最大その秒数だけ遅れます
